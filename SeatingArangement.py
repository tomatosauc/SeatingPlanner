...
def canvasReset(appStateInternal="Main menu", addInfo=None):
    ...
    match appStateInternal:
        case "Main menu":
            ...
            if len(addInfo) < 2:
                addInfo = ['', '']
            create_button(50, 300, "classButton", sizeX=200, sizeY=75, text="Výber triedy a skupiny")
        case "Main App":
            ...
            if addInfo is not None and addInfo[3]:
                ...
                try:
                    ...
                    subor = open(
                        os.path.expanduser('~') + '/zasadacie_poriadky/StudentLists/{}'.format(
                            addInfo[0]), 'r', encoding='utf-8')
                    meno = ''
                    for riadok in subor:
                        if Nriadok >= 0:
...
def create_button2(x, y, tags, sizeX=100, sizeY=50, text="", textScale=1.0, textOptions="", moreInfo=""):
    global actionBoxes2, canvas2
    if canvas2 is not None:
        canvas2.create_rectangle(x, y, x + sizeX, y + sizeY, tags=tags, width=2)
        canvas2.create_text(x + (sizeX / 2), y + (sizeY / 2), text=text,
                        font="Arial {} {}".format(round(15 * textScale * scale), textOptions), tags=tags + "_text")
        actionBoxes2.update(
            {tags: [[x, y, sizeX, sizeY, canvas2.itemcget(tags, "fill"), canvas2.itemcget(tags + "_text", "fill")],
                    moreInfo]})
...
def executeBox2(tags):
    global canvas2, currentTrieda, currentSkupina
    match tags:
        case 'BackButton':
            global canvas2, window2
            window2.destroy()
            canvas2 = None
    if 'class-' in tags:
        classesFile = [line for line in open(os.path.expanduser('~') + '/zasadacie_poriadky/StudentLists/Triedy.tssl', 'r', encoding='utf-8')]
        count = 0
        for classLine in classesFile:
            count += 1 if classLine[:2] == '--' else 0
        clas = tags.split('-')[1]
        skupiny = []
        for i, classLine in enumerate(classesFile):
            if classLine[:2] == '--':
                if classLine[2:].strip() == clas.upper():
                    i+=1
                    currentTrieda = classesFile[i].strip()
                    i+=1
                    while classesFile[i].strip() != 'END':
                        skupiny.append(classesFile[i][2:].strip())
                        i+=1
                    break
        skupiny.append('Žiadna')
        grid = []
        i = len(skupiny)
        while i > 0:
            grid.append(min(i,2))
            i -= min(i,2)
        skupina = 0
        for key in actionBoxes2.copy():
            if 'group-' in key:
                canvas2.delete(key)
                canvas2.delete(key + "_text")
                actionBoxes2.pop(key)
        for n, i in enumerate(grid):
            for j in range(i):
                create_button2(50+200/len(grid)*n, 80+count*75+40/i*j,tags = 'group-'+skupiny[skupina], text = skupiny[skupina], sizeX = 200/len(grid), sizeY = 40/i, textScale=1, moreInfo = skupiny[skupina])
                skupina += 1      
    elif 'group-' in tags:
        currentSkupina = actionBoxes2[tags][1]
        if currentSkupina == 'Žiadna':
            currentSkupina = ''
        executeBox2('BackButton')
        

def executeBox(tags):
    global canvas, binds, appState, textBox, tabulka, target, targetStudent, export, mode, trieda, canvas2, window2
    if binds != []:
        for bind in binds:
    ...
    match tags:
        case 'startButton':
            appState = 'Main App'
            canvasReset(appState, [currentTrieda, currentSkupina, '', True])
        case 'classButton':
            classesFile = [line for line in open(os.path.expanduser('~') + '/zasadacie_poriadky/StudentLists/Triedy.tssl', 'r', encoding='utf-8')]
            count = 0
            for classLine in classesFile:
            ...
            if canvas2 is None:
                ...
                canvas2 = tkinter.Canvas(window2, width='300', height=225+count*75)
                canvas2.pack()
                canvas2.create_text(150, 30, text='Výber triedy', font="Arial {} bold".format(round(25 * scale)),
                    anchor="center")
                triggerDefinition2()
                create_button2(50, 225+count*75-100, "BackButton", sizeX=200, sizeY=75, text="Späť", textScale=2, textOptions='bold')
                i=0
                for classLine in classesFile:
                    if classLine[:2] == '--':
                        className = classLine[2:].strip().capitalize()
                        create_button2(50, 60+i*75, tags='class-'+className, text=className, sizeX=200, sizeY=75, textScale=1.5, textOptions='bold')
                        i+=1
        case 'localMode':
            mode = 'Local'
        case 'dbMode':
        ...
        case 'export':
            ...
            for element in trieda:
                if element != '':
                    cas_a_trieda += "-" + (element[:-5] if 'tssl' in element else element)
            export.save(
...
def triggerDefinition2():
    global canvas2
    if canvas2 is not None:
        canvas2.bind('<Motion>', mouseMove2)
        canvas2.bind('<Button-1>', LMBClick2)
...
def mouseMove2(event):
    mouseX = event.x
    mouseY = event.y
    for box in actionBoxes2:
        if (
                actionBoxes2[box][0][0] < mouseX < actionBoxes2[box][0][0] + actionBoxes2[box][0][2] and
                actionBoxes2[box][0][1] < mouseY < actionBoxes2[box][0][1] + actionBoxes2[box][0][3]):
            canvas2.itemconfig(box, fill="#d0d0d0")
            canvas2.itemconfig(box + "_text", fill="#2F2F2F")
            canvas2.itemconfig(box + "_text2", fill="#2F2F2F")
        else:
            canvas2.itemconfig(box, fill=actionBoxes2[box][0][4])
            canvas2.itemconfig(box + "_text", fill=actionBoxes2[box][0][5])
            canvas2.itemconfig(box + "_text2", fill=actionBoxes2[box][0][5])
